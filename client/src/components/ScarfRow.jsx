 ...
 */
export default function ScarfRow({ scarf, photos, onDelete, onToggleFavorite }) {
  ...
  return (
    <>
      ...
      <div
        ...
        style={{ "--c1": scarf.color1 || "#c8102e", "--c2": scarf.color2 || "#fff" }}
      >
        {/* Colour stripe */}
        ...
        {photos.length > 0 ? (
          <div className="row-thumb-wrap" onClick={() => setLightbox(0)}>
            ...
            {photos.length > 1 && <span className="row-thumb-n">+{photos.length - 1}</span>}
          </div>
        ) : (
        ...
        <div className="row-info">
          ...
          {scarf.tags?.length > 0 && (
            <div className="row-tags-pills">
              ...
              {scarf.tags.length > 3 && (
                <span className="card-tag tag-more">+{scarf.tags.length - 3}</span>
              )}
            </div>
          )}
        </div>
